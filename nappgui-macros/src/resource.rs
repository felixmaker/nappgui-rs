...
impl Resource {
    ...
    fn get_sid(&self, uid: &str) -> Option<String> {
        self.uids.get(uid).map(|id| format!("N23R3C75::{}::{}", self.rid, id))
    }
    ...
    P: AsRef<Path>,
{
    ...
    let rid = dir.file_name().expect("Unable to get resource id from folder!");
...
fn generate_static_object(uid: &str, locale: &str, data: &ResourceData) -> String {
    ...
    match data {
        ResourceData::Message(message) => {
            format!("static {}_{}TEXT: &'static str = \"{}\";", uid, locale, message)
        }
...
fn generate_code(resource: &Resource) -> String {
    ...
    code.push(format!(
        "pub unsafe extern \"C\" fn {}_respack(locale: *const std::ffi::c_char) -> nappgui::core::ResPackPtr {{",
        resource.rid
    ));
    code.push("#[allow(unused)]".to_owned());
    code.push("let locale = unsafe { std::ffi::CStr::from_ptr(locale).to_str().unwrap() };".to_owned());
    code.push(format!(
